...
| 2026-08-21 | **推翻 Open Props 的引入方式**（Phase 5 審查發現）。原本整包 vendor 進 `assets/css/vendor/`（29KB），但實測全站只用到它 607 個變數中的 **13 個**，而檔案裡的 344 個色碼直接違反「色碼只出現在 theme.css」與「零第三方套件」兩條 Requirement，且當時是靠測試排除 `vendor` 目錄把落差藏起來。改為把那 13 個值直接寫進 `theme.css` 並標明出處，移除 vendor 目錄與測試豁免。**採用 Open Props 的比例仍然成立，改的是引入方式** |
| 2026-08-21 | **新增 `core/speech-text.js`**。「日文朗讀送假名而非漢字」這條規則原本在 `quiz-engine`、`vocab-view`、`grammar-view` 三處各寫一份，其中兩份在 ui 層因此測不到，寫法還不一致。收進 core 並補測試 |
| 2026-08-21 | **新增 `tests/structure.test.js`**。把「相對路徑」「零外部資源」「色碼集中」「文字對比度 ≥ 4.5:1」「core 層不得碰 DOM」「無巢狀 button」「題庫無重複物件鍵」七條原本只能靠 review 守的規格，變成自動化檢查。這讓 TD-5「只測 core 層」的覆蓋範圍往外擴了一圈 |
