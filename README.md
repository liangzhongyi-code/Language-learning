...
| 類型 | 檔案 | 測什麼 |
|---|---|---|
| 邏輯 | `tests/*.test.js` | 抽題、干擾選項、計分、統計、搜尋、語序推導、漢字顯示模式、逐題紀錄與間隔重複排程、出題範圍過濾、備份匯出匯入 |
...
| 鑰匙 | 內容 | 大小 |
|---|---|---|
| `lang-learn.stats.v1` | 「語言 × 題源」的答題計數器 | < 1 KB |
| `lang-learn.progress.v1` | 逐題的學習紀錄與複習排程 | 每筆約 59 bytes |
| `lang-learn.prefs.v1` | UI 偏好（漢字模式、假名顯示…） | < 1 KB |

統計與逐題紀錄刻意分開存：統計每次進語言首頁都要讀來畫一個百分比，
而逐題紀錄記滿全站一萬多個項目會到七百多 KB——合在一起的話，
畫一個百分比要先解析七百 KB 的 JSON。

三把鑰匙都能從首頁的「備份與還原」匯出成一個 JSON 檔再匯入回來。
沒有帳號、沒有雲端同步，換裝置靠那個檔案。
...
分兩把 key（理由見上面〈存在瀏覽器裡的三把鑰匙〉）：

- `lang-learn.stats.v1` — 以「語言 × 題型」為粒度累計（`ja:words`、`ja:cloze`、`en:reading`⋯）
- `lang-learn.progress.v1` — 逐題的作答次數、答錯次數、Leitner 盒號與下次到期時間

共通行為：

- **未完成的局兩邊都完全不計入**——作答到一半離開或重新整理就不算
- localStorage 被停用或已滿時會降級並回報失敗，測驗本身不受影響
- 語言首頁的「清除紀錄」會**同時清掉兩者**（且不分語言），需二次確認

間隔重複用 Leitner 盒子（間隔 1／3／7／16／35 天），答錯打回第一盒。
連續答對三次（爬到第 4 盒）就從「易錯」清單畢業——少了這個出口，錯誤次數只增不減，
一個字錯過一次就會永遠留在清單裡，練到全對數字也不會掉。
到期時間對齊當地日界而不是「加 N 個 24 小時」——後者會讓晚上答錯的題目
隔天整個上午都不到期，習慣早上練的人被系統性推遲一天。

> 兩個結構都帶版本號。日後若改格式，舊資料會被安全丟棄而不是造成錯誤——
> 也就是說**升級版本會清空既有紀錄**。
> 統計本身可以再練回來，但**複習排程是累積出來的、刪掉就回不去**，
> 所以首頁提供了匯出／匯入。
